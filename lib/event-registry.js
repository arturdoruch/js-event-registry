...
export default {
     ...
     * @param {jQuery|HTMLElement|string} element The jQuery or HTMLElement object, or CSS selector.
    ...
    on(events, element, listener, listenerArgs, listenerContext, preventDefault) {
        ...
        const id = Object.keys(_events).length + 1;
        _events[id] = event;
    ...
    /**
     * Removes event listener for event with id.
     *
...
function prepareEvent(events, element, listener, listenerArgs, listenerContext, preventDefault) {
    ...
    let event = new _Event(events, $element, listener, listenerArgs, listenerContext);
 ...
 */
function _Event(names, $element, listener, listenerArgs, listenerContext) {
    this.names = names;
